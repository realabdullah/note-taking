<script lang="ts" setup>
	...
	const emits = defineEmits<{
		(event: "setSecurityQuestions", securityQuestions: { question: string; answer: string }[]): void;
	}>();
	...
	const securityQuestions = ref([{ id: 1, question: "", answer: "" }]);

	const addSecurityQuestion = () => {
		securityQuestions.value.push({ id: Date.now(), question: "", answer: "" });
		emits(
			"setSecurityQuestions",
			securityQuestions.value.map(q => ({ question: q.question, answer: q.answer }))
		);
	};
	...
	const isQuestionSelected = (question: string, currentId: number) => {
		return securityQuestions.value.some(q => q.question === question && q.id !== currentId);
	};
</script>
				...
				<div class="space-y-2">
					...
					>
						...
						<option
							...
							:key="q"
							:value="q"
							:disabled="isQuestionSelected(q, question.id)"
						>
							{{ q }}
						</option>
					</select>

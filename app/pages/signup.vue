<script setup lang="ts">
	...
	const signUp = async () => {
		...
		successMessage.value = "Check your email to verify your address, then return to your workspace.";
	};
...
<template>
	<AuthPanel
		eyebrow="A workspace of your own"
	...
	>
		<form class="auth-form" @submit.prevent="signUp">
			...
			<button class="button button--primary" type="submit" :disabled="pending || Boolean(successMessage)">
				{{ pending ? "Preparing your workspace…" : "Create account" }}
			...
			<p class="auth-form__switch">Already use Fieldnote? <NuxtLink to="/login">Sign in</NuxtLink></p>
		</form>
	</AuthPanel>
</template>

	...
	<article class="editor paper" :class="{ 'editor--focus': isFocusMode }">
		<header class="editor__top">
			...
			<div class="editor__actions">
				<button
					class="editor__icon-button editor__focus-button"
					type="button"
				...
				>
					<Minimize2 v-if="isFocusMode" :size="17" aria-hidden="true" />
					<Maximize2 v-else :size="17" aria-hidden="true" />
				</button>
				<button class="editor__icon-button" type="button" aria-label="Share note" @click="handleShare">
		...
		<div class="editor__page">
			...
			<div class="editor__toolbar" aria-label="Markdown formatting">
				...
				<span class="mono">Markdown shortcuts · ⌘⇧F to focus</span>
			</div>
...
<style scoped>
	...
	.editor__page {
		...
		margin: 0 auto;
		padding: clamp(2rem, 5vw, 3.5rem) clamp(1.2rem, 5vw, 4.5rem) 5rem;
	}
	...
	.editor--focus .editor__page {
		width: min(1000px, calc(100% - 2rem));
		padding-top: clamp(3rem, 8vh, 6rem);
	}

	.editor--focus .editor__content {
		min-height: calc(100dvh - 15rem);
	}

	.editor__focus-button[aria-pressed="true"] {
